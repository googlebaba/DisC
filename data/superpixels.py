...
class SuperPixDatasetDGL(torch.utils.data.Dataset):
    def __init__(self, name, train_str, biased_test_str, unbiased_test_str, num_val=5000):
        """
        ...
        use_coord = True
        self.biased_test = SuperPixDGL("./data/superpixels", dataset=self.name, split=biased_test_str, 
                            use_mean_px=use_mean_px, 
                            use_coord=use_coord)
        self.unbiased_test = SuperPixDGL("./data/superpixels", dataset=self.name, split=unbiased_test_str, 
                            use_mean_px=use_mean_px, 
                            use_coord=use_coord)

        self.train_ = SuperPixDGL("./data/superpixels", dataset=self.name, split=train_str, 
                             use_mean_px=use_mean_px, 
                             use_coord=use_coord)
        ...
        with open("./data/superpixels/MNIST_BIASED.pkl", 'wb') as f:
            pickle.dump([self.train, self.val, self.biased_test, self.unbiased_test], f)
...
class SuperPixDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, name, data_dir):
        ...
        with open(data_dir+name+'.pkl',"rb") as f:
            ...
            self.train = f[0]
            self.val = f[1]
            self.biased_test = f[2]
            self.unbiased_test = f[3]
        li = list(range(len(self.train)))
        random.seed(10)
        random.shuffle(li)
        train1 = [self.train[n][0] for n in li[:10000]]
        train2 = [self.train[n][1] for n in li[:10000]]

        val1 = [self.val[n][0] for n in list(range(len(self.val)))]
        val2 = [self.val[n][1] for n in list(range(len(self.val)))]

        biased_test1 = [self.biased_test[n][0] for n in list(range(len(self.biased_test)))]
        biased_test2 = [self.biased_test[n][1] for n in list(range(len(self.biased_test)))]

        unbiased_test1 = [self.unbiased_test[n][0] for n in list(range(len(self.unbiased_test)))]
        unbiased_test2 = [self.unbiased_test[n][1] for n in list(range(len(self.unbiased_test)))]
        

        self.train = DGLFormDataset(train1, train2)
        self.val = DGLFormDataset(val1, val2)

        self.biased_test = DGLFormDataset(biased_test1, biased_test2)
        self.unbiased_test = DGLFormDataset(unbiased_test1, unbiased_test2)
        print('train, biased test, unbiased test, val sizes :',len(self.train),len(self.biased_test), len(self.unbiased_test),len(self.val))
    ...
    def _add_self_loops(self):
        ...
        self.biased_test.graph_lists = [self_loop(g) for g in self.biased_test.graph_lists] 
        self.unbiased_test.graph_lists = [self_loop(g) for g in self.unbiased_test.graph_lists]
        ...
        self.biased_test = DGLFormDataset(self.biased_test.graph_lists, self.biased_test.graph_labels)
        self.unbiased_test = DGLFormDataset(self.unbiased_test.graph_lists, self.unbiased_test.graph_labels)
